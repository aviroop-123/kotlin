...
private fun zipDirAs(dirPath: Path, zipFilePath: Path) {
    ...
    zipFilePath.outputStream().use { outputStream ->
        ZstdOutputStream(outputStream).use {
            TarArchiveOutputStream(outputStream).use { zipOutputStream ->
...
private inline fun TarArchiveOutputStream.newEntry(relativePath: String, isDir: Boolean, block: (TarArchiveEntry) -> Unit = {}) {
    val entry = if (isDir) {
